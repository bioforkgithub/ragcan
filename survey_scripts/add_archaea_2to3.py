#!/usr/bin/env python3
"""
GENUS_EXTENSION COPY (2026-09-17): identical to LPSN_Genus_Survey/scripts/add_archaea.py except
for --max-species (keep genera with min..max species) and a clash check that also covers the
survey's own genus list. Filters and tie-breaking are unchanged.
...
def main():
    ...
    keep={g:v for g,v in per.items() if len(v)>=a.min_species and (a.max_species is None or len(v)<=a.max_species)}
    ...
    survey=os.path.join(os.path.dirname(sg.ROOT),'LPSN_Genus_Survey','reports','survey_status.tsv')
    existing |= {r['genus'] for r in csv.DictReader(open(survey),delimiter='\t')}
